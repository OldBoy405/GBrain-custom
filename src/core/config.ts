...
export interface GBrainConfig {
  ...
  /**
   * DeepSeek API key. File-plane slot wired like openai/anthropic/zeroentropy:
   * `~/.gbrain/config.json` → loadConfig → buildGatewayConfig →
   * `DEEPSEEK_API_KEY` in the gateway env snapshot. Process env still wins.
   */
