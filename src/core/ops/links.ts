...
const traverse_graph: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    if (linkType === undefined && direction === undefined) {
      ...
      if (!excludePrivate) return nodes.slice(0, nodeLimit);
    ...
    if (!excludePrivate) return paths.slice(0, nodeLimit);
    const hiddenEdges = await findPrivateOnlySlugs(
      ctx.engine,
    ...
    return paths
      .filter(e => !hiddenEdges.has(e.from_slug) && !hiddenEdges.has(e.to_slug))
      .slice(0, nodeLimit);
  },
  scope: 'read',
