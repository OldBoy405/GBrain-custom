...
export function makeIngestCaptureHandler(engine: BrainEngine) {
  return async function ingestCaptureHandler(job: MinionJobContext): Promise<IngestCaptureResult> {
    ...
    // Inbox captures get workflow-state frontmatter prepared before the write;
    // all other captures pass content through unchanged.
    ...
    // Shared across the write and its FK-violation retry so provenance can not
    ...
    let result;
    try {
      result = await importFromContent(engine, slug, content, { ...importOpts, sourceId });
    } catch (err) {
      ...
      if (sourceId !== undefined && isSourceFk) {
        ...
        result = await importFromContent(engine, slug, content, { ...importOpts, sourceId: undefined });
      } else {
        throw err;
      }
    }
