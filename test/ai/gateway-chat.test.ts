...
describe('chat touchpoint — model resolver + aliases (Codex F-OV-5)', () => {
  ...
  test('assertTouchpoint accepts chat for chat-capable native + openai-compat providers', () => {
    ...
    expect(() => assertTouchpoint(getRecipe('deepseek')!, 'chat', 'deepseek-v4-pro')).not.toThrow();
  ...
  test('assertTouchpoint accepts expansion for deepseek v4', () => {
    expect(() =>
      assertTouchpoint(getRecipe('deepseek')!, 'expansion', 'deepseek-v4-flash'),
    ).not.toThrow();
    // openai-compat tier: undeclared expansion model ids still pass locally.
    expect(() =>
      assertTouchpoint(getRecipe('deepseek')!, 'expansion', 'deepseek-v4-pro'),
    ).not.toThrow();
  });
